...
final readonly class BootloaderGenerator implements GeneratorInterface
{
    ...
    private function updateInitConfigMethod(FileDeclaration $file, array $files): void
    {
        ...
        foreach ($files as $service) {
            ...
            $result[] = \sprintf(
                '%s::class => [\'host\' => $env->get(\'%s\', \'127.0.0.1:%d\')],',
                $className,
                (new EnvNameGenerator())->generate($namespacePrefix, $interfaceName),
                $port,
            );
            $port++;
        }
    ...
    private function createBootloader(string $namespace): FileDeclaration
    {
        ...
        $bootloader
            ->addMethod('defineSingletons')
            ->setReturnType('array')
            ->addBody(
                <<<'PHP'
                    return [
                        ServiceLocator::class => ServiceLocator::class,
        ...
        $bootloaderNamespace->addUse(AuthBootloader::class);
        ...
        $initMethod = $bootloader
            ...
            ->addBody(
                <<<'PHP'
                    ...
                    $interceptors = [
                        \Internal\Shared\gRPC\Interceptor\Incoming\RequestCollectorInterceptor::class,
                        \Internal\Shared\gRPC\Interceptor\Incoming\ContextInterceptor::class,
                        \Internal\Shared\gRPC\Interceptor\Incoming\ExceptionHandlerInterceptor::class,
                        \Internal\Shared\gRPC\Interceptor\Incoming\GuardInterceptor::class,
                        \Internal\Shared\gRPC\Interceptor\Incoming\OpenTelemetryInterceptor::class,
                        \Internal\Shared\gRPC\Interceptor\Incoming\DataGridInterceptor::class,
                    ];
        ...
        $initMethod
            ...
            ->setType(TokenizerListenerRegistryInterface::class);

        $initMethod
            ->addParameter('locator')
            ->setType(ServiceLocator::class);


        $bootloaderNamespace->addUse(Container::class);
        $bootloaderNamespace->addUse(GRPCBootloader::class);
        $bootloaderNamespace->addUse(TokenizerListenerRegistryInterface::class);
    ...
    private function removePrefixBeforeVersion(string $classPath): string
    {
        ...
        return \preg_replace_callback($pattern, static fn($matches) => // Return only the version part (v1 or v2)
        $matches[1], $classPath);
    }
}
