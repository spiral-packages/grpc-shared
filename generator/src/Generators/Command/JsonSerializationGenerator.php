...
final class JsonSerializationGenerator
{
    ...
    private function getJsonSerializeVariable(PropertyType $property): string
    {
        ...
        if ($property->isRepeatableByInitialType() && $property->isGoogleTimestampType()) {
            return \sprintf(
                '\array_map(static fn(\DateTimeInterface $item): string => $item->format("Y-m-d\TH:i:s.u\Z"), $this->%s)',
                $property->getCameCaseVariable(),
            );
        }
