    ...
    public function parse(string $dockblock): array
    {
        ...
        if (isset($matches[0])) {
            foreach ($matches[0] as $attribute) {
                ...
                if (\preg_match('/\#\[(?<class>[A-Za-z\\\]+)(\(\)){0,1}]/', (string) $attribute, $classMatches) === 1) {
                ...
                \preg_match('/\#\[(?<class>[A-Za-z\\\]+)\(/', (string) $attribute, $classMatches);
                $className = \str_replace(
                    \array_keys(self::CLASS_MAP),
                    \array_values(self::CLASS_MAP),
                    $classMatches['class'],
